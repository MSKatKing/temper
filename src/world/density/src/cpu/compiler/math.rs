...
use crate::cpu::compiler::{Compiler, compile};
...
fn compile_math_inner<R: RandomSource, P: PositionalRandom<R>>(
    ...
    let (left_value, left_buffer) = ValueSource::try_from(left, rand)
        ...
        .unwrap_or_else(|| {
            ...
            (ValueSource::Buffer(out), Some(out))
        });
    ...
    let (right_value, right_buffer) = ValueSource::try_from(right, rand)
        ...
        .unwrap_or_else(|| {
            ...
            let buffer = if let Some(left_buffer) = left_buffer
                && left_buffer == parent_buffer
            {
    ...
    ((left_value, left_buffer), (right_value, right_buffer))
}
...
macro_rules! implement_commutative {
    ($name:ident, $op:ident) => {
        pub fn $name<R: RandomSource, P: PositionalRandom<R>>(
            ...
            let ((left_value, left_buffer), (right_value, right_buffer)) =
...
pub fn compile_sub<R: RandomSource, P: PositionalRandom<R>>(
    compiler: &mut Compiler,
    rand: &mut P,
    parent_buffer: BufferId,
    left: &DensityFunctionArgument,
    ...
    match (left_buffer, right_buffer) {
        (Some(left_buffer), Some(right_buffer)) => {
            assert!(
                left_buffer.ty >= right_buffer.ty,
                "lhs size must be larger than or equal to rhs size"
            );
